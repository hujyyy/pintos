...
struct inode_disk
  {
    ...
    uint32_t unused[125-INODE_BLOCK_SIZE];     /* Not used. */
  };
  ...
  while (size > 0)
    {
      ...
      // if(sector_idx==3){
        //uint8_t tmp[512];
        //block_read (fs_device, sector_idx, tmp);
      // }else{
      ...
      //}
      //memcpy(buffer+bytes_read,entry->buffer+sector_ofs,chunk_size);
      //printf("cmp %d\n",memcmp(entry->buffer,tmp,BLOCK_SECTOR_SIZE) );
      ...
      //     //struct cache_entry* entry=cache_read(sector_idx,buffer+bytes_read);
      //     //memcpy(buffer+bytes_read,entry->buffer+sector_ofs,chunk_size);
      //   }
      // else
      //   {
