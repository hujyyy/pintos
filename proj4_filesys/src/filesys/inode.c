...
struct inode_disk
  {
    ...
    uint32_t unused[125-INODE_BLOCK_SIZE-6-1-1];     /* Not used. */
  };
...
inode_open (block_sector_t sector)
{
  ...
  block_read (fs_device, inode->sector, &inode->data);
  inode->parent_sector = inode->data.parent;
  return inode;
}
...
block_sector_t inode_number(struct inode* inode){
    if(inode==NULL) return -1;
    return inode->sector;
}

block_sector_t inode_parentsector(struct inode* inode){
    if(inode==NULL) return -1;
    return inode->parent_sector;
}

bool inode_set_parentsector(struct inode* parent_inode, block_sector_t sector){
  struct inode* inode = inode_open(sector);
  if(!inode) return false;
  inode->parent_sector = inode_number(parent_inode);
  inode->data.parent = inode_number(parent_inode);
  block_write(fs_device,inode->sector,&inode->data);
  inode_close(inode);
  //printf("%d\n",inode->parent_sector );
  return true;
