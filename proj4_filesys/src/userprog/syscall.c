...
syscall_handler (struct intr_frame *f)
{
  ...
  switch (sys_num) {
    ...
    /* Change the current directory. */
      check_memory(ptr+1);
      const char* dir_name = *(ptr+1);
      bool ret = syscall_chdir(dir_name);
      f->eax = ret;
      //printf("chdir %d\n",ret );
      return ret;
      // PANIC("xxx %s\n",dir_name );
    ...
    /* Create a directory. */
      check_memory(ptr+1);
      const char* dir_name = (char *)*(ptr+1);
      bool ret = syscall_mkdir(dir_name);
      f->eax = ret;
      return ret;
    ...
    /* Reads a directory entry. */
      ...
      int fd = *(ptr+4);
      char* name = *(ptr+5);
      bool ret = syscall_readdir(fd,name);
      f->eax = ret;
      return ret;
...
bool syscall_chdir(const char* pathname){
  ...
  if(dir!=NULL){
    if(dir_name==NULL){
      ...
      //printf("chdir %d %d\n",inode_number(dir_get_inode(dir)),inode_number(inode));
...
bool syscall_mkdir(const char* pathname){
  //if(flag) return false;
  //printf("%s\n", pathname);
  bool success = filesys_create(pathname,1,true);
  //printf("success %d\n", success );
  return success;
}
