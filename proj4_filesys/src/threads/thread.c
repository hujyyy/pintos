               ...
               thread_func *function, void *aux)
{
  ...
  if(tid!=-1)  t->parent = thread_current();
...
is_thread (struct thread *t)
{
  if(t->magic!=THREAD_MAGIC){
    //PANIC("not thread  %d,%d",t->magic,THREAD_MAGIC);
    PANIC("thread addr %x",t);
  }
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  t->pageTable = NULL;
  t->dir = NULL;
  //lock_init(&t->legacy_lock);
  //list_init(&t->mapping);
  //t->user_esp = 0;
  //t->ecode_table = malloc(sizeof(char*)*128);
  //for(int i =0;i<ECODE_TABLE_SIZE;i++) t->ecode_table[i] = (char)255;
...
struct thread* getthread(tid_t tid){
  ...
  for(tmp=list_begin(list);tmp!=list_end(list);tmp=list_next(tmp)){
    struct thread* thd =  list_entry (tmp, struct thread, allelem);
    if(thd->tid==tid) return thd;
  }
  //printf("%d\n");
  return NULL;
}
struct file* getfile(int fd){
  struct thread* cur = thread_current();
  struct list_elem* tmp;
  if(list_empty(&cur->files_list)) return NULL;
  for(tmp = list_begin(&cur->files_list);tmp!=list_end(&cur->files_list);tmp=list_next(tmp)){
    struct file_info* fi =  list_entry (tmp, struct file_info, file_elem);
    if(fi->fd == fd) return fi->file;
  }
  return NULL;

}
struct file_info* getfileinfo(int fd){
  struct thread* cur = thread_current();
  struct list_elem* tmp;
  if(list_empty(&cur->files_list)) return NULL;
  for(tmp = list_begin(&cur->files_list);tmp!=list_end(&cur->files_list);tmp=list_next(tmp)){
...
//using list to pass the exit status...
void push_legacy(void){
  ...
  //printf("%d %d\n",leg->tid,leg->exit_code );
  //list_remove(&leg->myelem);
  //lock_acquire(&cur->parent->legacy_lock);
  list_push_back(&cur->parent->children_legacy,&leg->myelem);
  //lock_release(&cur->parent->legacy_lock);
...
void free_legacylist(void){
  struct thread* cur = thread_current();
  struct list_elem* p;
  //lock_acquire(&cur->legacy_lock);
  while (!list_empty(&cur->files_list)) {
    p = list_pop_front(&cur->files_list);
    struct legacy* leg =  list_entry (p, struct legacy, myelem);
    free(leg);
  }
  //lock_release(&cur->legacy_lock);
  return;
}
...
int get_chilidlegacy(tid_t tid){
  ...
  struct list_elem* tmp;
  //lock_acquire(&cur->legacy_lock);
  if(!list_empty(&cur->children_legacy)){
